...
fun gotoAlipay():Boolean{
    if(isInstallAlipayClient()){
        App.current.startActivity(Intent.parseUri(URL_FORMAT,Intent.URI_INTENT_SCHEME))
        return true
    }
    return false
}
...
private fun isInstallAlipayClient() :Boolean{
    try{
        return App.current.packageManager.getPackageInfo(alipayClientPkgName,0) != null
